...
/** Nav items shared between sidebar and mobile menu. */
function NavItems({
  activeSection,
  onSectionClick,
}: {
  activeSection: ProfileSection;
  onSectionClick: (s: ProfileSection) => void;
}) {
  return (
    <>
      {PROFILE_SECTIONS.map((section) => (
        <button
          ...
          className={cn(
            "flex items-center gap-3 rounded-lg px-4 py-3 text-left text-sm font-sans transition-all duration-300",
    ...
    <div className="flex min-h-screen w-full flex-col bg-[var(--background)]">
      ...
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-white/10 bg-[var(--background)]/95 px-4 backdrop-blur-md lg:px-8">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2 text-sm text-white font-sans transition-colors hover:bg-white/10 lg:hidden"
        ...
        >
          ...
          <svg
            ...
            viewBox="0 0 12 12"
            fill="none"
            className={cn("transition-transform duration-300", isMobileMenuOpen && "rotate-180")}
          >
      ...
      <div className="flex flex-1">
        ...
        <aside className="hidden w-64 shrink-0 border-r border-white/10 lg:block">
          <nav className="sticky top-16 flex flex-col gap-1 p-4">
            <NavItems activeSection={activeSection} onSectionClick={handleSectionClick} />
          </nav>
        </aside>
        ...
        {/* Main content */}
        <main ref={contentRef} className="flex-1 overflow-y-auto px-4 py-6 lg:px-8 lg:py-10">
